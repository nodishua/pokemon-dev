...
class BaseIOStream(object):
    ...
    def _handle_events(self, fd, events):
        ...
        try:
            ...
            gen_log.info("Unsatisfiable read, closing connection(%s): %s" % (self.getpeername()[0], e))
            self.close(exc_info=True)
        except Exception:
            gen_log.error("Uncaught exception, closing connection(%s).",
                          self.getpeername()[0],
                          exc_info=True)
            self.close(exc_info=True)
            raise
    ...
    def _run_callback(self, callback, *args):
        def wrapper():
            ...
            try:
                ...
                app_log.error("Uncaught exception, closing connection(%s).",
                              self.getpeername()[0],
                              exc_info=True)
...
class IOStream(BaseIOStream):
    ...
    def __str__(self):
        try:
            s = '%s %s -> %s' % (object.__str__(self), str(self.socket.getpeername()), str(self.socket.getsockname()))
        except Exception:
    ...
    def _handle_connect(self):
        ...
        if err != 0:
            ...
            if self._connect_future is None:
                # too much log, by huangwei 20150910
