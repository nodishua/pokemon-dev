...
class Reader(Client):
    ...
    def connect_to_nsqd(self, host, port):
        ...
        if last_connect_attempt and last_connect_attempt > now - 3:
            return
    ...
    def _on_connection_close(self, conn, **kwargs):
        ...
        if not self.lookupd_http_addresses:
            ...
            logger.info('[%s:%s] attempting to reconnect in 5s', conn.id, self.name)
            reconnect_callback = functools.partial(self.connect_to_nsqd,
                                                   host=conn.host, port=conn.port)
            self.io_loop.add_timeout(time.time() + 5, reconnect_callback)
