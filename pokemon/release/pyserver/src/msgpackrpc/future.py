...
from tornado.concurrent import Future as torFuture
...
class Future(object):
    ...
    def set_result(self, result):
        ...
        self._set_flag = True
        self._tor_future.set_result(result)
    ...
    def set_error(self, err):
        if not isinstance(err, error.RPCError):
            err = error.CallError.from_msgpack(err)
        self.set(error=err)
        self._set_flag = True
        self._tor_future.set_exception(err)
