...
# CSV specification
# 1 The beginning is not left, and the behavior unit.
# 2 can contain or does not include the name, and the first line of the file file.
# 3 A line of data is not across the line, there is no vacancy.
# 4 The semi -corner comma (that is,) is used as a separator, which is listed as empty.
# 5 The content of the column is half -angle comma (that is,), the semi -angle quotation marks (that is, ',') include the value of the field.
# 6 If there is a semi -angle quotation (that is, "), the semi -angle dual quotation marks (" ") should be replaced, and this field value is contained with the semi -angle quotation marks (that is," ").
# 7 The quotation of the file is read and write, and the comma operation rules are against each other.
# 8 inner code format is not limited, which can be ascii, unicode or others.
# 9 Do not support special characters
...
TABLE_SLIM_REDUNDANCY = True # Lua table redundant optimization, for_lua can be turned on when it is true
VALUE_CROP = True # Cell content is cut according to Rule
...
# Even if the table ID is repeated, it will not re -generate the table
...
# Note that the watches that have never been changed, you cannot produce DIFF
def uuidname():
	global g_UUID
	g_UUID += 1
...
# 5. It is available for each field or no half -angle dual quotation marks (") include (but some programs, such as the Excel of Microsoft, do not need dual quotation). If the field is not included in the quotation marks, then the dual quotes cannot appear inside the field.character.
# 6. If the field contains a ride -to -line character, dual quotation or comma, the field needs to be included with a dual quotation number.
# 7. If a dual quotation section sub -field, then a dual quotation number must be added before the dual quotation in the field.
def strSys2User(s):
	s = s.strip()
	# remove system "
	quota = False
...
def isString(s):
	...
	if s[0] == "'" and s.count("'") == 2 and s[-1] == "'":
		return True
	# System string label, you also need subsequent inspections
	return False
...
def readXls(fileName):
	...
	for rx in xrange(sheet.nrows):
		...
		for cy in xrange(sheet.ncols):
			...
			# The first three lines are not processed, the following structural conversion is processed into a CSV storage structure
...
def parseCsv(fileName, keys, vars, defs, lines):
	# The first behavior variable name (must be available)
	...
	# (Or three) behavioral attribute names, ignore the empty column
	keyList = [x.strip() for x in (keys if isinstance(keys, list) else keys.split(','))]

	#Keys and VARS must be consistent, take a small value
	...
	# Below the starting line (such as _Comment, just comments function)
	# Definition in iGnore_keys
	...
	for i in xrange(1, len(varList)):
		...
		if varFilter and re.search(varFilter, varList[i]):
			# print fileName, varList[i], utf2local('Field ignore ')
			ignoreList[i] = True
		...
		if LANGUAGE and (fileName.find("StringCfg.xls") == -1 or fileName.find("StringCfg.xlsx") == -1):
			...
			if LANGUAGE == 'cn':
				...
				if l10nVar in varColMap:
					...
					l10nCheckedList[varColMap[l10nVar]] = True
					# There are multi -language versions directly replace the key name
	...
	# Ignore the first line as empty line
	validLines = []
	for line in lines:
	...
	for row in xrange(len(validLines)):
		for col in xrange(len(keyList)):
			...
			if ignoreList[col]:
				continue
			# The empty string is set to None. If you really need to be empty, you need to use Lua_String to define ''
	...
	# Judgment column data type
	for col in xrange(len(keyList)):
		luaType, defType = 0, 0
		# Get the default value firstlua type
		...
		for row in xrange(len(validMat)):
			...
			if validMat[row][col] == defList[col]:
				# If the string is the same, use the default value directly to reduce the configuration generation item
				validMat[row][col] = None
				continue
			elemType = whatType(validMat[row][col])
			luaType = max(luaType, elemType)

		# This column has the default value, strictly judged the type of value
		if defType != 0:
			for row in xrange(len(validMat)):
				if validMat[row][col] == None:
					continue
	...
	# The first column does not require variable names and default values
	luaType = keyList[0][2]
	keyList[0] = [keyList[0][0], None, luaType, None]

	# The first column can only be an integer.
	...
	for i in s:
		...
		if len(delim) > 0:
			if i == delim[-1]:
				...
				# There will be <and {possibilities in the string.
				pass
...
def makeArray(s):
	if not isArray(s):
		raise Exception('makeArray %s is invalid!' % s)
	...
	# Array needs to keep nil in the middle
...
def makeMap(s):
	if not isMap(s):
		raise Exception('makeMap %s is invalid!' % s)
...
# Now there is only the LUA_CSV value in the CSV with the relative directory
# When generating a file is the complete path, G_LUATableMap is considered to be a generated file when encountering a complete path
# @Return CSV path prefix structure, CSV path, CSV reference path
...
def makeElem(luaType, strValue):
	...
	if luaType == LUA_NIL:
		...
		# CSV is referenced using a string, and later is delayed in LUA
		ret = "'%s'" % value
		# print strValue, ret
...
def elemAppendComment(elem, p, src):
	p3 = elem.find('\r\n', p) + 2
	if p3 > p:
		comment = '\t\t-- %s\r\n\t' % src
		elem = elem[:p3] + comment + elem[p3+1:]
	else:
		comment = '\t--[[ %s]]' % src
		elem += comment
	p = p3 + len(comment)
		...
		for i, elem in enumerate(data):
			...
			while True:
				...
				if p < 0:
					break
				# find data key
				key = re.findall('(\w*) = __predefine_t__', elem[prep:])[0]
				...
				if (len(g_dataMap) > 150) and (dataKey not in g_dataMap):
					skipSlim = True
				...
				# 直接使用__predefine_t__
					...
					# Add notes that Table uses multiple times, pay attention to ELEM shorter, find \ r \ n Questions
					elem, p = elemAppendComment(elem, p - 15, src)
...
def value2elem(luaType, value, defaultValue):
	...
	if PY_NAMETUPLE:
		if value is None:
			...
			# Array and Map may return NONE, equivalent to lua_nil
			value = makeElem(luaType, value)
			# Optimize when the default value is
			if value == defaultValue:
				value = None
	else:
		# No data, no silent recognition, skipping generation
...
def makeLua(csvName, keyList, strMat, defList, hasRepeat):
	...
	for row in xrange(len(strMat)):
		...
		elemLine = strMat[row]
		if FOR_LUA:
			elem.append(LUA_ELEM_KV_FUNC("id", elemLine[0]))
		if VALUE_CROP:
			if languageIdx > 0 and relpath(csvName) not in ('cards', 'fragments'):
				# Determine whether the bank is an effective language
				if currentRowInvalid(LANGUAGE, elemLine[languageIdx], defList[languageIdx]):
					...
					continue
			# Determine whether the bank is effective
			if key > 0:
		...
		try:
			for col in xrange(1, len(keyList)):
				...
				if VALUE_CROP:
					value = rule.filterValue(relpath(csvName), keyList[col][1], value)
				# if FOR_LUA:
				# 	value = lua_x.encryptValue(relpath(csvName), keyList[col][1], value)
	...
	global g_dataMap # In order to view the convenience of DIFF, a table __data_t__Val [IDX1] = __predefine_t __ [IDX2]
	global g_dataMapLen # Records use the size of the definition space
	g_dataMap = {}
	g_dataMapLen = 0
	...
	if len(data) + g_dataMapLen > 10000:
		# lUA is too long, JIT, a certain version will report an error, Has More than 65536 Constants
		if FOR_LUA:
	...
	if FOR_LUA:
		module = module + "return %s" % varPath
...
def listFiles(rootDir, ext = None):
	...
	for root, dirs, files in list_dirs:
		for f in files:
			...
			if flag:
				list_ret.append(pf)
	# If it is list, there is a priority cover
	if isinstance(ext, list):
		listD = {}
...
def writeSrc(luaSrc):
	if FOR_LUA:
		luaSrc = ['lazy_require "%s"' % f for f in luaSrc]
		luaSrc = '\r\n'.join(luaSrc)
	else:
		luaSrc = '\r\n'.join(luaSrc)
...
def checkModify(fileList):
	...
	hasNewFile = False
	## whether there is a file deletion, if there is, or not under the Windows system, all re -generate
...
def main():
	...
	print (utf2local('csv2src Work list：')), os.getcwd()
	print (utf2local('csv2src Configuration source：')), SRC_PATH
	print (utf2local('csv2src Output file：')), LUA_FILE_NAME
	print (utf2local('csv2src language：')), LANGUAGE
	if not LANGUAGE:
		VALUE_CROP = False
	...
	# Without file addition and deletion, only modified files are generated
	if not hasNewFile:
	...
	for i, fileName in enumerate(fileList, 1):
		...
		if isinstance(lines[0], list):
			if lines[0][0].strip() != u'变量名':
				print (fileName, utf2local('The first line is not a "variable name", the file cannot be generated!'))
				continue
		else:
		...
		try:
			...
			if FOR_LUA:
				...
				while tmp:
					...
					paths.append(d)
				if paths[0].split('_')[-1] == LANGUAGE:
					paths[0] = '_'.join(paths[0].split('_')[:-1])
			...
			# raise
			print (fileName, utf2local('The file cannot be generated!(Check whether there are too many invalid columns)'))
	...
	print ('cost', datetime.datetime.now() - now)
