...
-- end
-- @function debug_hook
runner.debug_hook = nil
runner.debug_hook_init = function()
   if runner.debug_hook then return end
...
function runner.init(configuration)
   ...
   if runner.configuration.csmode then
      ...
      runner.sock = socket.tcp()
      runner.sock:setoption('keepalive', true)
      runner.sock:setoption('tcp-nodelay', true)
      local port = 1101
      local ret, err = runner.sock:connect("localhost", port)
      print('=== LuaCov connect', port, ret, err)
      if err ~= nil then
         error(err)
      end
      ...
      pdata = lnconv.lton(#pdata, 4) .. pdata
      local index, err, lastIndex = runner.sock:send(pdata)
      print('=== LuaCov init', #pdata, index, err, lastIndex)
      if err ~= nil then
         error(err)
      end
   end
