...
local zero_hits_exclusions = {
   ...
   "function x", -- "function Class(arg1, ..., argN)"
   "function x.x", -- "function Calss.Method(arg1, ..., argN)"
...
function ReporterBase:new(conf)
   ...
   print("=== LuaCov report to", conf.reportfile)
...
function DefaultReporter:on_new_file(filename)
   if self._cfg.onlysummary then return end

   self:write(("="):rep(78), "\n")
   self:write(filename, "\n")
   local stats = self:stats(filename)
   if stats.version then
      self:write(stats.version, "\n")
   end
   self:write(("="):rep(78), "\n")
end
