...
local function inflateGzip(input, output, bufsize, windowBits)
...
-- for easy
function _M.uncompress(comp)
    local readpos = 1
    local input = function(bufsize)
        -- Read the next chunk
        -- print('bufsize', bufsize)
        local ret = string_sub(comp, readpos, readpos + bufsize - 1)
    ...
    -- Decompress the data
    local ok, err = inflateGzip(input, output)
    if not ok then
        print('uncompress failed!', #comp, err)
        return
    end
