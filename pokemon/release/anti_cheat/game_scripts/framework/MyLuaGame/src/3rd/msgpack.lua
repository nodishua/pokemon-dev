...
local DEBUG_MSGPACK = false

-- modify by huangwei. 2014/7/24
-- >= 0x100000000 in 5.1.5 may be int overflow, but decimal will be ok
-- in cocos2dx luajit use hexadecimal is ok
local two52 = 4503599627370496
local two48 = 281474976710656
local two40 = 1099511627776
...
packers['double'] = function (buffer, n)
    ...
    elseif (mant == 0.0 and expo == 0) or expo < -0x3FE then
        ...
        expo = expo + 0x3FE -- IEEE 754, +1023 = 0x3FF
        mant = floor((mant * 2.0 - 1.0) * ldexp(0.5, 53))
        -- modify by huangwei. 2014/7/24
        buffer[#buffer+1] = char(0xCB,
                                 sign + floor(expo / 0x10),
                                 (expo % 0x10) * 0x10 + floor(mant / two48),
                                 floor(mant / two40) % 0x100,
                                 floor(mant / two32) % 0x100,
...
if DEBUG_MSGPACK then
    print ('msgpack: jit', jit)
    print ('msgpack: SIZEOF_NUMBER', SIZEOF_NUMBER)
end
