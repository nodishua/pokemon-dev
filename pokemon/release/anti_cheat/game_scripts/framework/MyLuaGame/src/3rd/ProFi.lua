...
local FORMAT_HEADER_LINE       = "| %-60s: %-40s: %-20s: %-12s: %-12s: %-12s: %-12s: %-12s|\n"
local FORMAT_OUTPUT_LINE       = "| %s: %-12s: %-12s: %-12s: %-12s: %-12s|\n"
...
function ProFi:pause()
	self.stopTime = getTime()
	self.runTime = self.runTime + self.stopTime - self.startTime
	self:stopHooks()
end

function ProFi:resume()
	self:startHooks()
	self.runTime = self.runTime + self.stopTime - self.startTime
	self.startTime = getTime()
end
...
function ProFi:setCallDeepMode( enable )
	self.deepMode = enable
end

function ProFi:setFlameMode( enable )
	self.flameMode = enable
end
...
function ProFi:writeProfilingReportsToFile( reports, file )
	local totalTime = self.stopTime - self.startTime + self.runTime
	local totalTimeOutput =  string.format(FORMAT_TOTALTIME_LINE, totalTime)
	file:write( totalTimeOutput )
	local header = string.format( FORMAT_HEADER_LINE, "FILE", "FUNCTION", "LINE", "SELFTIME", "TIME", "RELATIVE", "CALLED", "DEEP" )
	file:write( header )
 	for i, funcReport in ipairs( reports ) do
		local timer         = string.format(FORMAT_TIME, funcReport.timer)
		local selftimer     = string.format(FORMAT_SELFTIME, funcReport.selftimer)
		local count         = string.format(FORMAT_COUNT, funcReport.count)
		local deep          = string.format(FORMAT_DEEP, funcReport.maxDeep)
		local relTime 		= string.format(FORMAT_RELATIVE, (funcReport.timer / totalTime) * 100)
		local outputLine    = string.format(FORMAT_OUTPUT_LINE, funcReport.title, selftimer, timer, relTime, count, deep)
...
function ProFi:getCallDeep(currentLevel)
	currentLevel = currentLevel or 3
	local funcInfo = debug.getinfo( currentLevel, 'nS' )
	while funcInfo == nil do
		currentLevel = currentLevel - 1
		funcInfo = debug.getinfo( currentLevel, 'nS' )
	end
	while funcInfo do
		currentLevel = currentLevel + 1
		funcInfo = debug.getinfo( currentLevel, 'nS' )
...
function ProFi:onFunctionCall( funcInfo )
	...
	if self.flameMode then

	end
end

function ProFi:onFunctionReturn( funcInfo, parentFuncInfo )
	local funcReport = ProFi:getFuncReport( funcInfo )
	local parentFuncReport
	if parentFuncInfo then
		parentFuncReport = ProFi:getFuncReport( parentFuncInfo )
	end
	if funcReport.callTime then
		local cost = (getTime() - funcReport.callTime)
		funcReport.timer = funcReport.timer + cost
		funcReport.selftimer = funcReport.selftimer + cost
		if parentFuncReport then
			parentFuncReport.selftimer = parentFuncReport.selftimer - cost
		end
		if self.deepMode and funcReport.sampleDeep > 0 then
			funcReport.sampleDeep = funcReport.sampleDeep - 1
			funcReport.maxDeep = math.max(funcReport.maxDeep, self:getCallDeep(funcReport.sampleDeep))
		end
	end
end
...
onDebugHook = function( hookType )
	...
	elseif hookType == "return" then
		local parentFuncInfo = debug.getinfo( 3, 'nS' )
		ProFi:onFunctionReturn( funcInfo, parentFuncInfo )
	end
end
