...
def exec_coroutine(rpcServer):
	...
	Roles = {
		'game.cn.1': [
			('5de720ae5ec29639c547f7f9', 10114),
		],
	}
	...
	for role_id, uid in roles:
		...
		try:
			...
			if game:
				...
				logger.info('before fix_point %s role %d top6 %d top12 %d' % (server.key, uid, role.top6_fighting_point, role.top12_fighting_point))
				role.top6_fighting_point = 0
				role.top12_fighting_point = 0
				game.cards.onFightingPointChange()
				logger.info('after fix_point %s role %d top6 %d top12 %d' % (server.key, uid, role.top6_fighting_point, role.top12_fighting_point))
				...
				logger.info('fix_point %s role %d reset ok' % (server.key, uid))
				ok += 1
		except:
			logger.exception("fix_point %s role %d error" % (server.key, uid))
