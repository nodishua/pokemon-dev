...
GM_PATH = "/mnt/release_gm"

# 单服每5000开一个新服，下午5点之后满5000人则0点再开新服
# 0点判断一次开服条件。0-17点之间满5000开，其他时间不开
...
# cat en_24_game_server/2020-03-27.log|grep "new role"|wc -l
...
# key: game.en.1
def open_new_server(key):
	logger.info('open_new_server %s', key)

	# 1. 修改 game_defines
	kk = r"\.".join(key.split('.'))
	cmd = 'grep -n "%s" game_defines.py'% kk
	p = subprocess.Popen(cmd, cwd='/mnt/release_gm', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out, err = p.communicate()
...
def auto_open():
	...
	if current and nextserv:
		...
		maxc = ROLE_MAX_TO_OPEN
		logger.info('current %s new role count %d, max %d' , current, count, maxc)
		now = datetime.datetime.now()
		if now.hour >= 17:
			raise Return(None)
		if count > maxc:
			ding('准备开服', '1. 旧服角色已达 %d, 新服 %s 准备' % (count, nextserv))
			open_new_server(nextserv) # 开新服
