...
# cat kr_24_game_server/2020-03-27.log|grep "new role"|wc -l
...
def server_new_role_count(key):
	...
	cmd = 'cat /mnt/log/%s_%02d_game_server/*.log|grep "new role"|wc -l' % (tag, int(area))
...
def open_new_server(key, rolecount):
	...
	with cd("/mnt/release_gm"):
		...
		while True:
			...
			try:
				...
				ret = os.system('sed -i "%dc%s" game_defines.py'% (row+4, date))
				if ret != 0:
	...
	# 5. login release svn up
	ret = login_svn_up(c)
	with cd('/mnt/deploy_kr'):
		os.system('supervisorctl restart payment_server')
