...
class GeneralIndex(object):
    ...
    def _unindex(self, conn, pipe, id):
        ...
        if not known:
            return 0
        # modify by huangwei, support msgpack
        # old = json.loads(known.decode())
        old = msgpack.unpackb(known.decode())
        keys, scored = old[:2]
        pre, suf = ([],[]) if len(old) == 2 else old[2:]

        for key in keys:
            pipe.srem('%s:%s:%s'%(self.namespace, key, INDEX_NAME), id)
        for key in scored:
            pipe.zrem('%s:%s:%s'%(self.namespace, key, INDEX_NAME), id)
        for attr, key in pre:
            pipe.zrem('%s:%s:%s'%(self.namespace, attr, PREFIX_NAME), '%s\0%s'%(key, id))
        for attr, key in suf:
            pipe.zrem('%s:%s:%s'%(self.namespace, attr, SUFFIX_NAME), '%s\0%s'%(key, id))
    ...
    def index(self, conn, id, keys, scores, prefix, suffix, pipe=None):
        ...
        for key in keys:
            pipe.sadd('%s:%s:%s'%(self.namespace, key, INDEX_NAME), id)
        for key, score in scores.items():
            pipe.zadd('%s:%s:%s'%(self.namespace, key, INDEX_NAME), id, _to_score(score))
        for attr, key in prefix:
            pipe.zadd('%s:%s:%s'%(self.namespace, attr, PREFIX_NAME), '%s\0%s'%(key, id), _prefix_score(key))
        for attr, key in suffix:
            pipe.zadd('%s:%s:%s'%(self.namespace, attr, SUFFIX_NAME), '%s\0%s'%(key, id), _prefix_score(key))
        # pipe.hset(self.namespace + '::', id, json.dumps([list(keys), list(scores), list(prefix), list(suffix)]))
        pipe.hset(self.namespace + '::', id, msgpack.packb([list(keys), list(scores), list(prefix), list(suffix)]))
        if not had_pipe:
            pipe.execute()
        ...
        if len(filters) > 1:
            ...
            for fltr in filters:
                if isinstance(fltr, six.string_types):
                    pipe.scard('%s:%s:%s'%(self.namespace, fltr, INDEX_NAME))
                elif isinstance(fltr, Prefix):
                    estimate_work_lua(pipe, '%s:%s:%s'%(self.namespace, fltr.attr, PREFIX_NAME), fltr.prefix)
                elif isinstance(fltr, Suffix):
                    estimate_work_lua(pipe, '%s:%s:%s'%(self.namespace, fltr.attr, SUFFIX_NAME), fltr.suffix)
                elif isinstance(fltr, Pattern):
                    estimate_work_lua(pipe, '%s:%s:%s'%(self.namespace, fltr.attr, PREFIX_NAME), _find_prefix(fltr.pattern))
                elif isinstance(fltr, (tuple, list)):
                    pipe.zcard('%s:%s:%s'%(self.namespace, fltr[0], INDEX_NAME))
                else:
            ...
            if isinstance(fltr, list):
                ...
                if len(fltr) == 1:
                    ...
                    pipe.zunionstore(temp_id2, dict(
                        ('%s:%s:%s'%(self.namespace, fi, INDEX_NAME), 0) for fi in fltr))
                    intersect(temp_id, {temp_id:0, temp_id2:0})
                    pipe.delete(temp_id2)
            if isinstance(fltr, six.string_types):
                # simple string/tag search
                intersect(temp_id, {temp_id:0, '%s:%s:%s'%(self.namespace, fltr, INDEX_NAME):0})
            elif isinstance(fltr, Prefix):
                redis_prefix_lua(pipe, temp_id, '%s:%s:%s'%(self.namespace, fltr.attr, PREFIX_NAME), fltr.prefix, first)
            elif isinstance(fltr, Suffix):
                redis_prefix_lua(pipe, temp_id, '%s:%s:%s'%(self.namespace, fltr.attr, SUFFIX_NAME), fltr.suffix, first)
            elif isinstance(fltr, Pattern):
                redis_prefix_lua(pipe, temp_id,
                    '%s:%s:%s'%(self.namespace, fltr.attr, PREFIX_NAME),
                ...
                fltr, mi, ma = fltr
                intersect(temp_id, {temp_id:0, '%s:%s:%s'%(self.namespace, fltr, INDEX_NAME):1})
                if mi is not None:
    ...
    def search(self, conn, filters, order_by, offset=None, count=None, timeout=None):
        ...
        if order_by:
            ...
            order_clause = '%s:%s:%s'%(self.namespace, order_by.lstrip('-'), INDEX_NAME)
