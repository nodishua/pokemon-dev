...
class Column(object):
    ...
    def __init__(self, required=False, default=NULL, unique=False, index=False, keygen=None, prefix=False, suffix=False):
        ...
        # modify by huangwei, 2014/7/10.
        # Integer also convert to str, support Unique integer
        is_string = all(issubclass(x, six.string_types) or issubclass(x, six.integer_types) for x in allowed)
        if unique:
            if not is_string:
...
class Msgpack(Column):
    ...
    def _from_redis(self, value):
        if isinstance(value, self._allowed):
            return value
        return msgpack.unpackb(value, encoding='utf-8', object_pairs_hook=self._dict_pair)
